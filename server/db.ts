...
export async function initialiseTable() {
    ...
    try {
        ...
        await db.run("CREATE TABLE IF NOT EXISTS url(shortened TEXT, original TEXT)")
    } catch (e) {
        console.log(e)
    }
    return db;
...
export async function addShortenedUrl(db: Database, shortUrl: string, original: string): Promise<boolean> {
    try {
        await db.run(`INSERT INTO url(shortened, original) VALUES(?, ?)`, [shortUrl, original])
        return true
    } catch (e) {
        console.log(e);
        return false;
